...
void VectorTester::testInitialize() {
	funcname_ = "testInitizlie";
	Vector vec;
	vec.initialize();
	for (int i=0 ; i<100 ; ++i) {
		if (!vec.addElem(i)) {
			errorOut_("could not add 100 elements to initialized vector",1);
			break;
		}
	}
	passOut_("added 100 elements to initialized vector");
}
